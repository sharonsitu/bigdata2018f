...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      if (node.getAdjacencyList().size() > 0) {
        ...
        for (int i = 0; i < sources.length; i++) {
          float mass = pageranks.get(i) - (float) StrictMath.log(list.size());
          pageranks.set(i,mass);
        }
  ...
  private static class ReduceClass extends
    ...
    protected void setup(Context context) throws IOException, InterruptedException {
      super.setup(context);
      this.sources = context.getConfiguration().getStrings(SOURCES);
      totalMasses = new float[sources.length];
    ...
    public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
      ...
      if (structureReceived == 1) {
        ...
        for (int j = 0; j < sources.length; ++j) {
          totalMasses[j] = sumLogProbs(totalMasses[j],masses[j]);
        }
  ...
  private static class MapPageRankMassDistributionClass extends
    ...
    public void setup(Context context) throws IOException {
      ...
      String[] m = conf.getStrings("MISSING_MASSES");
      missingMasses = new float[m.length];
      for (int i = 0; i < m.length; ++i) {
        missingMasses[i] = Float.parseFloat(m[i]);
      }
    ...
    public void map(IntWritable nid, PageRankNode node, Context context)
      ...
      ArrayListOfFloatsWritable pageranks = node.getPageRanks();
      for (int i = 0; i < sources.length; ++i) {
        float p = pageranks.get(i);
        float jump = Float.NEGATIVE_INFINITY;
        float link = Float.NEGATIVE_INFINITY;
        int source = Integer.parseInt(sources[i]);
        // all mass lost in the dangling nodes are put back into source node
        if (nid.get() == source) {
          float missingmass = missingMasses[i];
          // random jump is always going back to source node, so p(jump) = alpha/1
          // All dangling nodes masses are put into source node
          jump = (float) Math.log(ALPHA);
          link = (float) Math.log(1.0f - ALPHA)
                  + sumLogProbs(p, (float) (Math.log(missingmass)));
        } else {
          // p(jump) = 0
          link = p + (float) (Math.log(1.0f - ALPHA));
        }
  ...
  public int run(String[] args) throws Exception {
    ...
    LOG.info(" - num sources: " + sourceList);
  ...
  private float[] phase1(int i, int j, String basePath, int numNodes,
    ...
    for (int m = 0; m < masses.length; ++m) {
      masses[m] = Float.NEGATIVE_INFINITY;
    }
    FileSystem fs = FileSystem.get(getConf());
    for (FileStatus f : fs.listStatus(new Path(outm))) {
      FSDataInputStream fin = fs.open(f.getPath());
      for (int n = 0; n < masses.length; ++n) {
        masses[n] = sumLogProbs(masses[n], fin.readFloat());
      }
      fin.close();
    }
  ...
  private void phase2(int i, int j, float[] missing, String basePath, int numNodes, String[] sources) throws Exception {
    ...
    String[] missinglist = new String[missing.length];
    for (int k = 0; k < missing.length; k ++) {
      missinglist[k] = Float.toString(missing[k]);
    }

    job.getConfiguration().setBoolean("mapred.map.tasks.speculative.execution", false);
    job.getConfiguration().setBoolean("mapred.reduce.tasks.speculative.execution", false);
    job.getConfiguration().setStrings("MISSING_MASSES", missinglist);
    job.getConfiguration().setInt("NodeCount", numNodes);
    job.getConfiguration().setStrings(SOURCES,sources);
